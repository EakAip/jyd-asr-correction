...
# Remote model config
# =========================
# REMOTE_LLM_URL = os.environ.get("REMOTE_LLM_URL", "http://188.18.54.100:5001/v1/workflows/run").strip()
# REMOTE_LLM_URL = os.environ.get("REMOTE_LLM_URL", "http://120.211.237.163:5001/v1/workflows/run").strip()
# 怀来调用本机大模型需要改为内网地址
# REMOTE_LLM_URL = os.environ.get("REMOTE_LLM_URL", "http://10.6.26.163:5001/v1/workflows/run").strip()
REMOTE_LLM_URL = os.environ.get("REMOTE_LLM_URL", "http://188.18.18.149:5001/v1/workflows/run").strip()
REMOTE_LLM_MODEL = os.environ.get("REMOTE_LLM_MODEL", "JYD").strip()
# REMOTE_LLM_API_KEY = os.environ.get("REMOTE_LLM_API_KEY", "app-u0ktTSKupEArQdRCQzEHr5IW").strip()
# REMOTE_LLM_API_KEY = os.environ.get("REMOTE_LLM_API_KEY", "app-8ixHPWXYG9tnSCEoH685aMWF").strip()
REMOTE_LLM_API_KEY = os.environ.get("REMOTE_LLM_API_KEY", "app-MWrE04oEPloukfxztcATJfbo").strip()
REMOTE_TIMEOUT = float(os.environ.get("REMOTE_LLM_TIMEOUT", "60"))

if not REMOTE_LLM_URL:
    raise ValueError(
        "REMOTE_LLM_URL is empty. Please set environment variable REMOTE_LLM_URL."
    )
...
logger.info(f"Remote llm url: {REMOTE_LLM_URL}")
...
# Regex constants
# =========================
_RELAXED_INTENT_RE = re.compile(
    r"INTENT_ID\s*=\s*(\d+)\s*(;\s*SEEK\s*=\s*YES)?",
    re.I
)
...
def normalize_generation_output(raw_result: str) -> Tuple[str, Optional[Dict[str, Any]]]:
    """
    返回: (cleaned_result, extra_data)
    - cleaned_result: 标准化后的字符串 (如 "INTENT_ID=41")
    - extra_data: 额外的字段 (如 {"lyric": "我家小姐桂英"})
    """
    raw = (raw_result or "").strip()

    # 尝试解析 JSON 格式 {"intent_id":24, "lyric": "xxx"}
    try:
        parsed = json.loads(raw)
        if isinstance(parsed, dict) and "intent_id" in parsed:
            intent_id = parsed["intent_id"]
            # 提取除 intent_id 外的其他字段
            extra_data = {k: v for k, v in parsed.items() if k != "intent_id"}
            return f"INTENT_ID={intent_id}", extra_data if extra_data else None
    except (json.JSONDecodeError, ValueError):
        pass

    matches = list(_RELAXED_INTENT_RE.finditer(raw))
    if matches:
        last = matches[-1]
        intent_id = last.group(1)
        seek_yes = bool(last.group(2))
        return f"INTENT_ID={intent_id}" + (";SEEK=YES" if seek_yes else ""), None

    m = re.fullmatch(r"\s*(\d+)\s*", raw, re.S)
    if m:
        return f"INTENT_ID={m.group(1)}", None

    return raw.strip(), None


def extract_final_intent(cleaned_result: str) -> Tuple[int, bool]:
    matches = list(_RELAXED_INTENT_RE.finditer(cleaned_result or ""))
    if matches:
        last = matches[-1]
        intent_id = int(last.group(1))
        seek_yes = bool(last.group(2))
        return intent_id, seek_yes

    m = re.fullmatch(r"\s*(\d+)\s*", cleaned_result or "", re.S)
    if m:
...
def extract_remote_text(resp_json: Dict[str, Any]) -> str:
    ...
    if isinstance(data, dict):
        ...
        if isinstance(outputs, dict):
            # 优先检查 outputs.result（Dify workflow 格式）
...
def call_remote_llm(
    ...
    logger.info(f"Remote LLM request url: {REMOTE_LLM_URL}")
    logger.info(f"Remote LLM request model: {REMOTE_LLM_MODEL}")
    logger.info(f"Remote LLM request text: {repr(text)}")
    ...
    try:
        ...
        logger.warning(f"Remote response is not valid JSON: {resp.text[:1000]}")
        return resp.text.strip()

    raw_result = extract_remote_text(resp_json)
    if not raw_result:
        logger.warning(f"Remote JSON parsed but no text extracted: {resp_json}")
        raw_result = json.dumps(resp_json, ensure_ascii=False)
...
def call_remote_llm(
    ...
    logger.info(f"Remote LLM request model: {REMOTE_LLM_MODEL}")
    logger.info(f"Remote LLM request text: {repr(text)}")
    logger.info(f"Remote LLM workflow inputs: {payload['inputs']}")
...
def build_intent_service(
    model_name: str = REMOTE_LLM_MODEL,
    ...
    logger.info(f"Using remote model via POST: {REMOTE_LLM_URL}")
    logger.info(
        f"Remote generation config | model={model_name}, "
    ...
    def service(req: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            ...
            cleaned_result, extra_data = normalize_generation_output(raw_result)

            logger.info(f"LLM input text: {repr(text)}")
            logger.info(f"LLM raw result: {repr(raw_result)}")
            logger.info(f"LLM cleaned result: {repr(cleaned_result)}")
            logger.info(f"LLM extra data: {extra_data}")

            intent_id, seek_yes = extract_final_intent(cleaned_result)

            # 后置校验: LLM 对无法识别的输入常兜底误判为 43(返回上一级)。
            # 若返回 43 但原文没有对应关键词, 判定为无效指令, 降级为 0。
            if intent_id == 43 and not contains_any(
                normalize_text(text), ["返回", "回去", "上一页", "上一级", "退回", "退出去"]
            ):
                logger.info(
                    f"Intent 43 without keyword, downgraded to 0 | text={repr(text)}"
                )
                intent_id = 0

            data: Dict[str, Any] = {"intent_id": intent_id}

            # 合并额外字段（如 lyric）
            if extra_data:
                data.update(extra_data)

            # Intent 29: 跳转至指定时间点 - 从原始文本中提取时间
            if intent_id == 29:
                jump_time = extract_jump_time(text)
                if jump_time:
                    data["minutes"] = jump_time["minutes"]
                    data["seconds"] = jump_time["seconds"]
                    # 移除大模型可能返回的 time 字段
                    data.pop("time", None)
                    logger.info(f"Jump time parsed | text={repr(text)} | minutes={data['minutes']} | seconds={data['seconds']}")

            if intent_id in (9, 10) and seek_yes:
                seek_time, unit = extract_seek_info(text)
                if seek_time is not None and unit is not None:
                    data["seek_time"] = int(seek_time)
                    data["unit"] = unit
                    logger.info(
                        f"Seek parsed | text={repr(text)} | seek_time={data.get('seek_time')} | unit={data.get('unit')}"
                    )

            logger.info(
                f"Parsed intent | text={repr(text)} | intent_id={intent_id} | seek_yes={seek_yes}"
            )
            ...
            logger.error(f"[REMOTE_LLM_HTTP_ERROR] {type(e).__name__}: {str(e)}")
...
# 启用 CORS 跨域支持 - 允许浏览器从任何源访问此服务
CORS(app, resources={
    r"/*": {
        "origins": "*",  # 允许所有域名访问（生产环境建议改为具体域名）
